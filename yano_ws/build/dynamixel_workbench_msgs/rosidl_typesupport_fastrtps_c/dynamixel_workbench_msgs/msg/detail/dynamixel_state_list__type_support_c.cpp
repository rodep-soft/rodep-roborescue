  ...
  void * untyped_ros_message)
{
  ...
  // Field name: dynamixel_state
  {
    ...
    if (!dynamixel_workbench_msgs__msg__DynamixelState__Sequence__init(&ros_message->dynamixel_state, size)) {
      fprintf(stderr, "failed to create array for field 'dynamixel_state'");
      return false;
    }
    auto array_ptr = ros_message->dynamixel_state.data;
    for (size_t i = 0; i < size; ++i) {
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = dynamixel_workbench_msgs__msg__DynamixelStateList;
    is_plain =
      (
      offsetof(DataType, dynamixel_state) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _DynamixelStateList__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_dynamixel_workbench_msgs__msg__DynamixelStateList(
