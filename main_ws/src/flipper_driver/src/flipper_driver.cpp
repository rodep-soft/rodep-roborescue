  ...
  FlipperDriver() : Node("flipper_driver") {
    ...
    if (!initDynamixel()) {
      RCLCPP_ERROR(this->get_logger(),
                   "Failed to initialize Dynamixel Workbench");
      rclcpp::shutdown();
    }

    subscription_ =
        this->create_subscription<custom_interfaces::msg::DriverVelocity>(
            "/flipper_driver", 10,
            std::bind(&FlipperDriver::driver_callback, this, _1));

    estop_subscription_ = this->create_subscription<std_msgs::msg::Bool>(
        "/emergency_stop", 10,
        std::bind(&FlipperDriver::estop_callback, this, _1));
...
private:
  ...
  bool initDynamixel() {
    ...
    for (const auto &id : dynamixel_ids_) {
      if (!dxl_wb_.ping(id)) {
        RCLCPP_ERROR(this->get_logger(),
                     "Failed to ping Dynamixel motor with ID %ld", id);
        return false;
      }
    ...
    RCLCPP_INFO(this->get_logger(),
                "Dynamixel Workbench initialized successfully");
    return true;
  }
  ...
  void driver_callback(const custom_interfaces::msg::DriverVelocity &msg) {
    if (estop_active_) {
      RCLCPP_WARN(this->get_logger(),
                  "Emergency stop is active. Ignoring velocity commands.");
      return;
    }

    std::vector<int32_t> velocities = {msg.flipper_vel[0], msg.flipper_vel[1],
                                       msg.flipper_vel[2], msg.flipper_vel[3]};
  ...
  void estop_callback(const std_msgs::msg::Bool::SharedPtr msg) {
    ...
    if (estop_active_) {
      RCLCPP_WARN(this->get_logger(),
                  "Emergency stop activated. Stopping all motors.");
      stopMotors();
    } else {
      RCLCPP_INFO(this->get_logger(),
                  "Emergency stop deactivated. Resuming motor control.");
    }
  }
